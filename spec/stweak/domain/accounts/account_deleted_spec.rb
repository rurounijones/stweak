...
RSpec.describe Stweak::Domain::Accounts::AccountDeleted do
  subject(:event) do
  ...
  it 'reports its type and version' do
    expect([event.type, event.version]).to eq(['AccountDeleted', 1])
  end

  it 'requests owner-key shredding' do
    expect(described_class.shreds_owner_key?).to be(true)
  end
