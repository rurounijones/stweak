...
# A subclass, to pin that equality is by exact class: an account id and a
# subclass instance with the same value are unequal, and unequal both ways.
...
RSpec.describe Stweak::Domain::Accounts::AccountId do
  ...
  it 'is not equal to a subclass with the same value, symmetrically' do
    ...
    expect([account_id == other, other == account_id]).to all(be(false))
  end
  ...
  it 'is not equal to a generic id with the same value, symmetrically' do
    generic = Stweak::Domain::Id.new(value: uuid)
    expect([account_id == generic, generic == account_id]).to all(be(false))
  end
end
