...
require_relative '../../../../lib/stweak/domain/accounts/disable_account_handler'
require_relative '../../../../lib/stweak/domain/accounts/disable_account'
...
def disable_created_events(account_id, count)
...
def disable_created_checkpoint
...
RSpec.describe Stweak::Domain::Accounts::DisableAccountHandler do
  subject(:handler) do
  ...
  let(:command) { Stweak::Domain::Accounts::DisableAccount.new(account_id: account_id) }

  before do
    allow(checkpoint_store).to receive(:get).and_return(nil)
    allow(checkpoint_store).to receive(:put)
    allow(event_store).to receive(:read_stream).and_return(disable_created_events(account_id, 1))
    allow(event_store).to receive(:append)
  end

  it 'returns the disabled account' do
    expect(handler.handle(command).disabled).to be(true)
  end

  it 'appends an AccountDisabled event' do
    handler.handle(command)
    expect(event_store).to have_received(:append)
      .with(hash_including(events: [an_instance_of(Stweak::Domain::Accounts::AccountDisabled)]))
  end
  ...
  it 'reads only the tail after the checkpoint version' do
    allow(checkpoint_store).to receive(:get).and_return(disable_created_checkpoint)
    handler.handle(command)
    expect(event_store).to have_received(:read_stream)
  ...
  it 'persists the checkpoint at a boundary' do
    allow(event_store).to receive(:read_stream).and_return(disable_created_events(account_id, 99))
    handler.handle(command)
  ...
  it 'surfaces a concurrent append as AccountAlreadyDisabled' do
    allow(event_store).to receive(:append).and_raise(Stweak::Ports::ConcurrencyError)
    expect { handler.handle(command) }.to raise_error(Stweak::Domain::Accounts::AccountAlreadyDisabled)
  end
end
