...
RSpec.describe Stweak::Adapters::CheckpointStore::InMemoryCheckpointStore do
  ...
  it 'copies nested state so mutating it after storing does not leak in' do
    roles = ['admin']
    store.put(owner_type: ACCOUNT_TYPE, owner_id: owner_id, checkpoint: checkpoint({ 'roles' => roles }, 100))
    roles << 'root'
    expect(store.get(owner_type: ACCOUNT_TYPE, owner_id: owner_id).state).to eq('roles' => ['admin'])
  end
