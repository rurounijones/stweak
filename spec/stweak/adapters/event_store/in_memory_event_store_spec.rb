...
# A VersionedEvent in its version-1 state, so a store spec can append an old
# shape and prove it is upcast on read without the stored form being rewritten.
def build_v1_event(stream_id)
...
RSpec.describe Stweak::Adapters::EventStore::InMemoryEventStore do
  ...
  it 'upcasts an older-version event on read' do
    store.append(owner_type: owner_type, stream_id: account_id, expected_version: 0,
                 events: [build_v1_event(account_id)])
    read = store.read_stream(owner_type: owner_type, stream_id: account_id).first
    expect(read).to have_attributes(name: 'alice', version: 2)
  end

  it 'does not rewrite the appended event' do
    old = build_v1_event(account_id)
    store.append(owner_type: owner_type, stream_id: account_id, expected_version: 0, events: [old])
    expect(old.to_h).to include('full_name' => 'alice', 'version' => 1)
  end
