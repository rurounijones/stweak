...
RSpec.shared_examples 'a projection store' do
  ...
  it 'merges written cursors into those already stored' do
    subject.write(projection_name: 'AccountsProjector', cursors: cursors)
    ...
    expect(subject.read(projection_name: 'AccountsProjector')).to eq(cursors.merge(later))
  end

  it 'advances a stored cursor when its stream is written again' do
    subject.write(projection_name: 'AccountsProjector', cursors: cursors)
    subject.write(projection_name: 'AccountsProjector', cursors: { stream_key => 2 })
    expect(subject.read(projection_name: 'AccountsProjector')).to eq(stream_key => 2)
  end
