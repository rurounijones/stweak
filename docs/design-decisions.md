...
per-stream cursors in a shared `projection_cursors` table. A projector writes
its materialized rows and advances its cursors in one transaction, so a failed
batch leaves both at their previous state and at-least-once delivery can retry
it safely.
