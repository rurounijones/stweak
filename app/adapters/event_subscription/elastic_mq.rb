...
module App
  module Adapters
    ...
    # rubocop:disable Metrics/ClassLength -- the poller-thread tracing (poll and
    # deliver spans plus cross-thread trace-context propagation) lives inline
    # here because it cannot be added from outside; that is what tips it over.
    class ElasticMqSubscription
      ...
      # rubocop:disable Metrics/MethodLength -- the span wrapper and per-message
      # rescue add the lines; the poll itself is small.
      sig { void }
      def poll_once
        @tracer.in_span(
      ...
      #
      # @param body [String]
      # rubocop:disable Metrics/MethodLength -- the consumer-span wrapper and
      # trace-context reopening add the lines; the delivery loop is small.
