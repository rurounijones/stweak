...
RSpec.describe App::Adapters::ProjectionStore::EncryptingProjectionStore do
  ...
  it 'passes transactions through' do
    store.transaction { store.read(projection_name: 'Missing') }
    expect(store.read(projection_name: 'Missing')).to be_nil
  end

  it 'wraps a row and cursor write in one transaction' do
  ...
  it 'passes cursor advances through' do
    store.write(projection_name: 'AccountsProjector', cursors: { 'Account#1' => 1 })
    store.advance(projection_name: 'AccountsProjector', cursors: { 'Account#2' => 2 })
    expect(store.read(projection_name: 'AccountsProjector')).to eq('Account#1' => 1, 'Account#2' => 2)
  end

  it 'does not encrypt cursor advances' do
    store.advance(projection_name: 'AccountsProjector', cursors: { 'Account#1' => 1 })
    expect(raw_store.read(projection_name: 'AccountsProjector')).to eq('Account#1' => 1)
  end

  it 'does not create a cursor projection for an empty advance' do
    store.advance(projection_name: 'AccountsProjector', cursors: {})
    expect(store.read(projection_name: 'AccountsProjector')).to be_nil
  end

  it 'does not move a cursor backwards through the decorator' do
    store.write(projection_name: 'AccountsProjector', cursors: { 'Account#1' => 1 })
    store.advance(projection_name: 'AccountsProjector', cursors: { 'Account#1' => 0 })
    expect(store.read(projection_name: 'AccountsProjector')).to eq('Account#1' => 1)
  end
