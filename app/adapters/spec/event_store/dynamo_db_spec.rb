...
# A listener that records deliveries, so the store's publishing can be asserted.
...
def list_tables_page(names: [], next_page: nil)
  instance_double(
    Aws::DynamoDB::Types::ListTablesOutput,
    table_names: names, last_evaluated_table_name: next_page
  )
end
...
RSpec.describe App::Adapters::DynamoDBEventStore do
  ...
  it 'checks all ListTables pages when finding an existing table' do
    allow(client).to receive(:list_tables).and_return(
      list_tables_page(next_page: 'page-one'), list_tables_page(names: ['stweak_streams'])
    )
    allow(client).to receive(:create_table)
    described_class.new(client: client, streams_table: 'stweak_streams')
    expect(client).not_to have_received(:create_table)
  end
  # rubocop:enable RSpec/ExampleLength

  # rubocop:disable RSpec/ExampleLength
  it 'creates a table after no ListTables page has a match' do
    allow(client).to receive(:list_tables).and_return(
      list_tables_page(next_page: 'page-one'), list_tables_page
    )
    allow(client).to receive(:create_table)
    described_class.new(client: client, streams_table: 'stweak_streams')
    expect(client).to have_received(:create_table)
  end
  # rubocop:enable RSpec/ExampleLength

  it 'creates a missing table after ListTables finds no match' do
    allow(client).to receive(:list_tables).and_return(list_tables_page)
    allow(client).to receive(:create_table)
    described_class.new(client: client, streams_table: 'stweak_streams')
    expect(client).to have_received(:create_table)
  end

  it 'ignores a concurrent table creation after ListTables' do
    allow(client).to receive(:list_tables).and_return(list_tables_page)
    allow(client).to receive(:create_table).and_raise(
      Aws::DynamoDB::Errors::ResourceInUseException.new({}, 'in use')
    )
    expect { described_class.new(client: client, streams_table: 'stweak_streams') }.not_to raise_error
  end
