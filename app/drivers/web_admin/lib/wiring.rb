...
module WebAdmin
  ...
  # (populated from .env; see .env.example) and assembles a read-only Reader
  # over the chosen stores. Unlike the other drivers it is config-driven rather
  # than hard-wired, because switching adapters from a .env is the point of this
  # one. dotenv populates ENV; this class still reads ENV.fetch, so only the
  # source of the vars is new, not the way they are read. It never touches the
  # write side — no subscription, no hasher, no checkpoint store.
  class Wiring
    extend T::Sig
