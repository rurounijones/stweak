...
Mutation testing and the dependency checks are slower on purpose and are left
out of the default task. Run them when the work warrants it:

    bundle exec rake mutant
    bundle exec rake audit
    bundle exec rake licenses
...
- **Dependency audit** — `bundle exec rake audit` runs `bundler-audit` over
  the bundled gems and `ruby-audit` over the Ruby and RubyGems the bundle runs
  on, both against the shared ruby-advisory-db, so a known CVE at either level
  fails. Slow and network-bound, so outside the default task and the hooks.
- **Licences** — `bundle exec rake licenses` runs `license_finder`, failing if
  any dependency carries a licence not approved in
  `config/dependency_decisions.yml`. Each approval there records why it was
  granted; the config lives in `config/license_finder.yml`.
...
and licence check, Markdown and typos, and actionlint. The pinned versions
there match the devcontainer's, so what passes locally is what runs in CI.
