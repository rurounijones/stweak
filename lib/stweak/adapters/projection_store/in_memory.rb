...
module Stweak
  module Adapters
    ...
    module ProjectionStore
      ...
      # hash of primary key to row. Nothing survives a restart, which is
      # accepted while the domain gem has no durable collaborator of its own.
      class InMemoryProjectionStore
        ...
        # rubocop:disable Naming/BlockForwarding -- Sorbet requires a named block
        ...
        def write(projection_name:, cursors:)
          @cursors[projection_name] = cursors
        end
        ...
        def advance(projection_name:, cursors:)
          ...
          stored = @cursors[projection_name] ||= {}
          cursors.each do |stream_key, sequence|
            stored[stream_key] = sequence if stored.fetch(stream_key, 0) < sequence
          end
        end
        ...
        def upsert(table:, attributes:)
          rows = rows_for(table)
          rows[attributes.fetch(primary_key(table))] = attributes
        end
