...
module Stweak
  module Ports
    ...
    module ProjectionStore
      ...
      # Record the given per-stream cursors, merging them into any already
      # stored: a stream present in both takes the written sequence, and a
      # stream absent from this call keeps its stored cursor. The projection
      # system persists only the streams a batch advanced, so a write carries a
      # delta rather than the whole map, and cursors only ever move forward. The
      # store keeps a snapshot of the cursors rather than retaining the caller's
      # hash, so a later in-place advance of the passed hash is not observed as
      # if it had been persisted.
      #
      # @param projection_name [String] the projection's stable name
      # @param cursors [Hash{String => Integer}] the advanced streams, each
      #   mapped to the highest sequence applied
