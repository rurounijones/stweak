...
module Stweak
  module Ports
    ...
    module ProjectionStore
      ...
      # Run projection row and cursor changes in one transaction.
      #
      # @yield the operations that must commit atomically
      # rubocop:disable Naming/BlockForwarding -- Sorbet requires the named block
      ...
      # Replace a projection's stored cursors.
      #
      # @param projection_name [String] the projection's stable name
      ...
      # Merge newly advanced cursors into a projection's stored cursors. A
      # supplied position lower than the stored position is ignored.
      #
      # @param projection_name [String] the projection's stable name
      # @param cursors [Hash{String => Integer}] streams advanced by processing
      #   the current batch
      sig { abstract.params(projection_name: String, cursors: T::Hash[String, Integer]).void }
